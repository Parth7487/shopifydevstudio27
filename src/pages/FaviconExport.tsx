    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      <div className="max-w-lg w-full bg-zinc-900/70 border border-white/10 rounded-2xl p-6">
        ...
        <p className="text-gray-300 mb-6">
          Download the site icon in multiple formats and sizes for best quality.
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <button
            onClick={() => exportAll("image/png")}
            className="px-4 py-3 rounded-lg bg-beige text-black font-semibold hover:opacity-90"
          >
            Download PNG (512/192/64)
          </button>
          <button
            onClick={() => exportAll("image/jpeg")}
            className="px-4 py-3 rounded-lg border border-beige text-beige font-semibold hover:bg-beige/10"
          >
            Download JPEG (512/192/64)
          </button>
        </div>
        <div className="mt-6 text-sm text-gray-400">
          SVG source is already served at{" "}
          <code className="text-gray-200">/favicon.svg</code>.
        </div>
      </div>
    </div>
  );
